...
#     # Thumb (check x for horizontal direction)
#     fingers.append(1 if lm_list[4].x < lm_list[3].x else 0)
...
#             finger_states = get_finger_states(lm_list)

#             for gesture_name, (command, pattern) in GESTURE_COMMANDS.items():
#                 if finger_states == pattern:
#                     if last_executed != gesture_name or time.time() - last_time > 3:
#                         run_command(command)
